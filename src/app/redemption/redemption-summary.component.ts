...
export class RedemptionSummaryComponent implements OnInit, OnDestroy {
    ...
    constructor(private route: ActivatedRoute, private router: Router, private logger: Logger,
                private redemptionService: RedemptionService, private http: Http, private dataStorageService: DataStorageService,
    ...
    back() {
        this.urlAccessGuard.allow = true;
        let isInstaRedemption = this.dataStorageService.get(this.config.isRedemptionInsta);
        if (!isNullOrUndefined(isInstaRedemption) && isInstaRedemption){
            this.router.navigate(['instant'], {relativeTo: this.route.parent});
        }
        else {
    ...
    ngOnDestroy() {
        ...
        this.dataStorageService.remove(this.config.isRedemptionInsta);
    }
}